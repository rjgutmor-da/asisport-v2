...
BEGIN
  ...
  -- SuperAdministrador puede gestionar toda su escuela sin asignacion docente.
  -- Alcance solicitado por el usuario el 2026-09-11; los otros roles no cambian.
  IF v_rol = 'SuperAdministrador' THEN
    v_target_entrenador_id := p_entrenador_id;
  ELSIF v_rol IN ('Administrador', 'Asistente') THEN
  ...
  WITH candidatos AS (
    ...
    WHERE a.escuela_id = v_escuela_id
      ...
      AND (
        (v_rol = 'SuperAdministrador' AND v_target_entrenador_id IS NULL)
        OR (v_rol = 'Entrenarqueros' AND a.es_arquero IS TRUE)
  ...
  IF v_rol = 'SuperAdministrador' THEN
    -- Contar los registros de la lista visible, independientemente de su autor.
    SELECT count(*) INTO v_total_asistencias_fecha
    FROM jsonb_array_elements(v_asistencias) AS asistencia
    WHERE EXISTS (
      SELECT 1 FROM jsonb_array_elements(v_candidatos) AS candidato
      WHERE candidato->>'id' = asistencia->>'alumno_id'
    );
  ELSE
    SELECT COUNT(*)
